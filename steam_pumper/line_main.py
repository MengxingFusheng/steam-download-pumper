from __future__ import annotations

import logging
import os
import signal
import sys

from .line_config import save_line_config
from .line_controller import LineController
from .line_web import run_line_web


def main() -> int:
    config_path = os.environ.get("CONFIG_PATH", "/data/config.json")
    config_exists = os.path.exists(config_path)
    controller = LineController(config_path)
    if not config_exists:
        save_line_config(config_path, controller.cfg)
    logging.basicConfig(level=getattr(logging, controller.cfg.log_level.upper(), logging.INFO), format="%(asctime)s %(levelname)s %(message)s")

    def stop(_signum: int, _frame: object) -> None:
        controller.shutdown()
        raise SystemExit(0)

    signal.signal(signal.SIGTERM, stop)
    signal.signal(signal.SIGINT, stop)
    controller.start_scheduler()
    run_line_web(controller, "0.0.0.0", int(os.environ.get("WEB_PORT", "80")))
    return 0


if __name__ == "__main__":
    sys.exit(main())
